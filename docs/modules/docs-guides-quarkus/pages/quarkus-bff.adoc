...
=== OpenApi generator
After creating your OpenAPI file in the right xref:project-structure.adoc[ directory ] :
...
See more at <<_security_permissions>>
...
. Create a package called `mockserver` inside your test resources package.
See  xref:project-structure.adoc[project structure] for more information.

. Create an empty file called `internal.json` in this package.
This file will be used by the mockserver to save expectations whenever you create one inside your tests.

. Create a file called `mockserver.properties` inside your test.resources package.
...
See more in link:https://github.com/onecx/onecx-theme-bff/blob/main/src/test/java/org/tkit/onecx/themes/bff/rs/ThemeRestControllerTest.java[GitHub Theme BFF reference] for an example test class.
